...
import { UserHistoryModal } from './components/UserHistoryModal'; // Importar UserHistoryModal
...
function DashboardPage({ onNavigateToAdmin }: DashboardPageProps) {
  ...
  const [showHistoryModal, setShowHistoryModal] = useState(false); // State para o Histórico
    ...
    try {
      ...
      try {
          ...
          const { error: saveError } = await api.insert('news', {
              ...
              tipo: mode,
              author_id: user.id,
              sources: sources || [] // Agora salva as fontes corretamente
          });

          if (saveError) {
             console.error("Erro ao salvar no histórico (Supabase):", saveError);
             
             // Detecta o erro específico de ENUM inválido
             if (saveError.includes('invalid input value for enum') || saveError.includes('news_type')) {
                 const fixCommand = "ALTER TABLE news ALTER COLUMN tipo TYPE text; DROP TYPE IF EXISTS news_type;";
                 setToast({ 
                     message: "ERRO DE BANCO DE DADOS: O tipo de conteúdo é novo e o banco não reconhece. Rode este SQL no Supabase: " + fixCommand, 
                     type: 'error' 
                 });
                 // Copia para o clipboard para facilitar
                 navigator.clipboard.writeText(fixCommand);
                 console.warn("SQL SUGERIDO:", fixCommand);
             }
             // Detecta erro de coluna faltando
             else if (saveError.includes('column') && saveError.includes('author_id') && saveError.includes('does not exist')) {
                 setToast({ 
                     message: "ERRO DE BANCO DE DADOS: A tabela 'news' precisa ser atualizada. Execute o SQL de migração.", 
                     type: 'error' 
                 });
             } else {
                 setToast({ message: "Aviso: O conteúdo foi gerado, mas falhou ao salvar no histórico. Erro: " + saveError, type: 'error' });
             }
          }
  ...
  return (
    <div className="min-h-screen bg-black text-gray-300">
      <Header 
        ...
        onOpenHistory={() => setShowHistoryModal(true)} // Passando a nova prop
